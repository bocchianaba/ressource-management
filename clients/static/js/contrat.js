...
function showContract(contractName, clientName, providerName, projectDescription, projectBudget, contractDuration, maxChanges) {
  ...
  popup.innerHTML = '<h3>' + contractName + '</h3><p><strong>Client:</strong> ' + clientName + '</p><p><strong>Prestataire:</strong> ' + providerName + '</p><p><strong>Description du Projet:</strong> ' + projectDescription + '</p><p><strong>Budget du Projet:</strong> ' + projectBudget + '</p><p><strong>Durée du Contrat:</strong> ' + contractDuration + '</p><p><strong>Nombre Max de Modifications:</strong> ' + maxChanges + '</p><button class="button" onclick="printContract()">Imprimer</button>';
